...
export class UcatLearningReport implements OnInit, AfterViewInit, OnDestroy {
  ...
  readonly filteredCourses = computed(() => {
    ...
    // 1. Tab filter (All, In Progress, Completed)
    if (tab === 'in_progress') {
      filtered = filtered.filter((item) => item.rawStatus === 'in_progress');
    } else if (tab === 'completed') {
      filtered = filtered.filter((item) => item.rawStatus === 'completed');
    }

    // 2. Active Category filter (from chips / filter dropdown)
    if (activeCats.length > 0) {
      filtered = filtered.filter((item) => {
        return activeCats.some((cat) => {
          const c = cat.toLowerCase();
          return (
            item.type.toLowerCase().includes(c) ||
            item.category.toLowerCase().includes(c) ||
            item.stageInfo.toLowerCase().includes(c) ||
            item.title.toLowerCase().includes(c)
          );
        });
      });
    }
    ...
    return filtered.sort((a, b) => {
      // 1. Primary ordering when on 'all' tab: Status priority (IN_PROGRESS -> NOT_STARTED -> COMPLETED)
      if (tab === 'all') {
  ...
  private normalizeStatus(
    ...
    rawStatus: 'not_started' | 'in_progress' | 'completed';
    statusDisplay: 'Not Started' | 'In Progress' | 'Completed';
  } {
    const raw = (status || '').trim().toLowerCase().replace(/[\s-_]+/g, '_');
    ...
    if (
      ...
      return { rawStatus: 'completed', statusDisplay: 'Completed' };
    }
    ...
    if (
      ...
      return { rawStatus: 'in_progress', statusDisplay: 'In Progress' };
    }

    return { rawStatus: 'not_started', statusDisplay: 'Not Started' };
  }
  ...
  private mapReportItem(item: UcatLearningReportItem): UcatCourseItem {
    ...
    const activeSession =
      ...
      item.session_id ||
      null;
    const { rawStatus, statusDisplay } = this.normalizeStatus(
      item.status,
      progressPercent,
      activeSession
    );
